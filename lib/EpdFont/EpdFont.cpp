...
void EpdFont::getTextBounds(const char* string, const int startX, const int startY, int* minX, int* minY, int* maxX,
  ...
  *maxY = startY;
  ...
  while ((cp = utf8NextCodepoint(reinterpret_cast<const uint8_t**>(&string)))) {
    ...
    const combiningMark::Anchor markAnchor =
        isCombining ? combiningMark::anchorFor(cp) : combiningMark::Anchor::CenterRaised;
    const int raiseBy =
        isCombining ? combiningMark::raiseAboveBase(markAnchor, glyph->top, glyph->height, lastBaseTop) : 0;
    ...
    const int glyphBaseX =
        isCombining ? combiningMark::anchorOver(markAnchor, lastBaseX, lastBaseLeft, lastBaseWidth, glyph->left,
                                                glyph->width, prevCp)
                    : lastBaseX;
